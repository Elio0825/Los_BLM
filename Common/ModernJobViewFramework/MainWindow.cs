...
public class MainWindow {
  ...
  public void MainControlView(ref bool buttonValue, ref bool stopButton, Action save) {
    ...
    if (_smallWindow) {
      ...
      ImGui.PushStyleVar(ImGuiStyleVar.ItemSpacing, new Vector2(5, 2));
    } else {
      // 正常模式：使用标准内边距
  ...
  private void DrawTopControlBar(ref bool buttonValue, ref bool stopButton, Action save) {
    ...
    float scale = QtStyle.OverlayScale;
    
    // 计算所需的最小宽度（统一使用标准尺寸）
    float mainButtonWidth = 100 * scale;
    float controlButtonsWidth = 96 * scale; // 两个按钮 + 间距
    float minRequiredWidth = mainButtonWidth + controlButtonsWidth + 20 * scale;
    
    // 主控制按钮
    DrawMainControlButton(ref buttonValue, ref stopButton);

    // 检查窗口宽度是否足够，如果不够则换行显示
    bool shouldWrap = windowWidth < minRequiredWidth;
    
    if (shouldWrap) {
      // 窗口太小，换行显示右侧按钮
      // 在小窗口模式下，减少间距以避免占用过多空间
      if (!_smallWindow) {
        ImGui.Spacing();
      }
      ...
      // 右侧控制按钮组
      ImGui.SameLine(windowWidth - controlButtonsWidth);
      DrawControlButtons(save);
    }
  ...
  private void DrawControlButtons(Action save) {
    ...
    // 根据窗口大小调整按钮间距
    float spacing = _smallWindow ? 8f : 5f;
  ...
  private void DrawInfoSection() {
    ...
    if (_smallWindow) {
      // 小窗口模式：显示紧凑版的时间轴和职能设置
      DrawCompactInfoSection(currTriggerLine);
    } else {
      // 正常模式：显示完整的提示卡片
  ...
  /// 绘制紧凑的信息区域（小窗口模式）
  /// </summary>
  private void DrawCompactInfoSection(TriggerLine? currTriggerLine) {
    // 检查是否显示小窗信息
    if (GlobalSetting.Instance == null || !GlobalSetting.Instance.小窗显示时间轴和职能) {
      return;
    }
    ...
    string notice = currTriggerLine != null 
    ...
    // 计算职能设置标签和下拉框的总宽度
    ImGui.SameLine();
    Vector2 labelSize = ImGui.CalcTextSize("职能设置");
    float comboValueWidth = 50f * scale; // D4等值显示的宽度
    float totalWidth = labelSize.X + comboValueWidth + 10f * scale; // 10f是间距
    float rightPadding = 8f * scale;
    
    // 计算右对齐位置
    float startX = windowWidth - totalWidth - rightPadding;
    ...
    // 职能设置下拉框
    ImGui.SetNextItemWidth(comboValueWidth);
    ImGui.PushStyleVar(ImGuiStyleVar.FramePadding, new Vector2(4, 4));
    if (ImGui.BeginCombo("##role",
